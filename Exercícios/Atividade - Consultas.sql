...
# Exercício - Lista 3
select * from clientes where inscricao_estadual is null;
select nome_vendedor, salario_fixo from vendedores order by nome_vendedor asc;
select nome_produto, valor_unitario from produtos where unidade_medida = 'KG' order by valor_unitario asc;
update vendedor set salario_fixo = salario_fixo * 175 + 120;
select nome_vendedor, salario_fixo from vendedores order by nome_vendedor asc;

# Exercício - Lista 3 continuação
SELECT MIN(salarioFixo) AS menorSalario, MAX(salarioFixo) AS maiorSalario FROM vendedor;
SELECT SUM(qtd) AS quantidadeTotal FROM itempedido WHERE codProd = 78; 
SELECT AVG(salarioFixo) AS mediaSalarios FROM vendedor;
SELECT COUNT(*) AS totalVendedores FROM vendedor WHERE salarioFixo > 2500.00;
SELECT DISTINCT descricao AS unidadesProdutos FROM produto;
SELECT codPedido, COUNT(*) AS quantidadeProdutos FROM itempedido GROUP BY codPedido;
SELECT codPedido FROM itempedido GROUP BY codPedido HAVING COUNT(*) > 3;

# Exercício - Lista 4
SELECT * FROM vendedor WHERE faixaComissao <> 'B';
SELECT * FROM vendedor WHERE salarioFixo IS NULL;
SELECT nomeCli, Endereco FROM cliente ORDER BY nomeCli ASC;
SELECT nome, salarioFixo FROM vendedor ORDER BY salarioFixo DESC;
SELECT codProd, qtd FROM itempedido WHERE qtd > 20;
SELECT nome, salarioFixo * 1.5 AS novoSalarioFixo FROM vendedor ORDER BY nome ASC;
SELECT codPedido, COUNT(*) AS quantidadeProdutos FROM itempedido GROUP BY codPedido; SELECT MIN(codPedido) AS menorPedido, MAX(codPedido) AS maiorPedido FROM itempedido;
SELECT SUM(qtd) AS quantidadeTotal FROM itempedido WHERE codProd = 31;
SELECT COUNT(*) AS totalVendedores FROM vendedor WHERE salarioFixo < 1500.00;
